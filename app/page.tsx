...
import { STATIONS, WATERSHEDS, LAKE_FACTS, SEEDING_CONTEXT, HYPSOMETRY } from "@/lib/data";
...
export default function Page() {
  ...
  for (const s of inflowSites) {
    const v = sites[s.id]?.params?.["00060"]?.value;
    if (v !== undefined && !isNaN(v)) { totalCfs += v; counted++; }
  }
  const afPerDay = totalCfs * 1.9835; // 1 cfs ≈ 1.9835 AF/day
  const afPerYear = afPerDay * 365;

  // exposed-bed delta if level moved 1 ft (illustrative)
  const sqMiPerFt = HYPSOMETRY.dAreaPerFtSqMi;
    ...
    <div className="app">
      <aside className="sidebar">
        <div className="brand">
          ...
          <div className="sub">live USGS gauges · watershed physics · seeding context</div>
        ...
        <div className="section">
          ...
          <div className="note">
            {saltair ? "Live from USGS gage 10010000." : "Showing last reported value (live fetch pending)."} Healthy range 4,198–4,205 ft (UT DNR).
          </div>
        </div>
        ...
        <div className="section">
          ...
          <div className="note">Gauges sit several miles upstream of the lake; true at-lake inflow differs (last-mile historically unmeasured — new gauges being added). Annualized figure assumes today's flow held constant, so it's a snapshot, not a forecast.</div>
        </div>
        ...
        <div className="section">
          ...
          <div className="note">
            Catchment total ≈ {fmt(LAKE_FACTS.catchmentSqMi)} sq mi. All inflow originates as precipitation across these basins; rivers concentrate it. ~{LAKE_FACTS.directPrecipSharePct}% of input falls directly on the lake. Runoff efficiency is gated by antecedent soil moisture — dry soils absorb snowmelt before it reaches the lake.
          </div>
        </div>
        ...
        <div className="section">
          ...
          <div className="note">
            Against a {fmt(LAKE_FACTS.oneTimeDeficitAF / 1e6, 0)}M-AF deficit and {fmt(LAKE_FACTS.structuralShortfallAF / 1000, 0)}K-AF/yr structural shortfall, bottom-up seeding volumes are 2–3 orders of magnitude too small to materially refill the lake. The defensible value of seeded water is in agriculture & ski pathways, not lake replenishment. ~1 ft of level ≈ {sqMiPerFt} sq mi of lakebed (coarse).
          </div>
        </div>
