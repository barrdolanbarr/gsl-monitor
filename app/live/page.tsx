...
export default function LivePage() {
  ...
  const afPerDay = totalCfs * 1.9835;
  ...
  return (
    <div className="app">
      <aside className="sidebar">
        <div className="brand">
          <h1>Bear River Basin — Live Monitor</h1>
          <div className="sub">live USGS gauges · snow-aware water balance · seeding hotspots</div>
        ...
        <div className="section">
          ...
          <div className="statgrid">
            ...
            <div className="stat"><div className="k">Bear at Corinne</div><div className="v">{fmt(cfsOf("10126000"), 0)}<small>cfs</small></div></div>
          </div>
        ...
        <div className="section">
          ...
          <div className="note">
            The Bear is the largest tributary to the Great Salt Lake — ~{WATERSHEDS.find((w) => w.short === "Bear")?.inflowSharePct ?? 58}% of the lake's river inflow. Its basin drains ≈ 7,500 sq mi across Utah, Idaho, and Wyoming.
          </div>
        </div>

        {/* SEEDING IMPACT CONTEXT */}
        <div className="section">
          <h2>Cloud-seeding generation context</h2>
          ...
          <div className="seed-row"><span>Net reaching the GSL</span><b>{fmt(SEEDING_CONTEXT.toLakeLowAF)}–{fmt(SEEDING_CONTEXT.toLakeHighAF)} AF/yr</b></div>
          <div className="note">
            The value of generated water is overwhelmingly in what it does <em>on the way down</em> — farms, cities, hydropower, habitat. See the <a href="/" style={{ color: "var(--accent)" }}>Overview</a> for the downstream-value picture.
          </div>
        </div>
        ...
        {/* GSL TERMINAL — small context card */}
        <div className="section">
          <h2>Great Salt Lake — terminal</h2>
          <div className="hero">
            <span className="val">{fmt(lakeElev, 2)}</span>
            <span className="unit">ft (Saltair)</span>
          </div>
          <div className="bar"><div className="fill" style={{ width: `${pct}%` }} /></div>
          <div className="scale"><span>4,188</span><span>4,205</span></div>
          <div className="note">
            Where Bear River water ends up if it isn't diverted first. {saltair ? "Live USGS gauge 10010000." : "Last reported value."} How much of new water reaches the lake — and what that's worth — is on the <a href="/gsl" style={{ color: "var(--accent)" }}>Reaches the GSL</a> page.
          </div>
        </div>
        ...
        {model && (
          <div className="section">
            <h2>Snow-aware water balance — pilot</h2>
            <div className="statgrid">
            ...
            <div className="seed-row"><span>Seeding → river (p50)</span><b>{fmt(model?.seeding_central?.delta_af_at_outlet)} AF/yr</b></div>
            ...
            <div className="note">
              Snow-aware water balance, calibrated to {model?.meta?.gauge_years} yrs of gauge data (gauge {model?.meta?.outlet_gauge}, Bear River near Corinne).
            </div>
          </div>
        )}
        ...
        {grid && (
          <div className="section">
            ...
            <div className="note">
              Toggle layers in the top-right of the map. {grid?._meta?.n_cells} cells across {grid?._meta?.elev_min_m}–{grid?._meta?.elev_max_m} m.
            </div>
          </div>
        )}

        <div className="foot">
          Data: USGS National Water Information System (waterservices.usgs.gov), public/no-key. Elevation: open-elevation (SRTM/ASTER). Land cover: NLCD 2021 (MRLC/USGS). Seeding context: bottom-up estimate from Rainmaker disclosed validation — illustrative, not company figures.
        </div>
      </aside>
